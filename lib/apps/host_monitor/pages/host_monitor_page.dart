        ...
        itemBuilder: (_, index) => HostCards(
          ...
          onDelete: () async {
            ...
            try {
              ...
              AppLogger.debug(
                '[HostEditPage] ${_tr('host_deleted')}: ${host.name}',
              );
              ...
              AppLogger.error('[HostEditPage] ${_tr('delete_failed')}', e);
              if (mounted) {
                CustomSnackBar(
                  context,
    ...
    if (mounted) {
      if (_provider.errorMessage != null &&
        ...
        await showAdvancedConfirmDialog(
          context: context,
          style: ConfirmDialogStyle.material,
          title: _tr('connection_timeout'),
          content:
              '${_tr('unable_to_connect_to')} ${host.name}\n${_tr('please_check_network')}',
        ...
        await showAdvancedConfirmDialog(
          context: context,
          style: ConfirmDialogStyle.material,
          title: _tr('authentication_failed'),
          content:
              '${_tr('token_verification_failed_for')} ${host.name}\n${_tr('please_check_token')}',
