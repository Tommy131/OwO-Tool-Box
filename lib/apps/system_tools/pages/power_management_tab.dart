...
class _PowerManagementTabState extends State<PowerManagementTab> {
              ...
              children: [
                Expanded(
                  child: OutlinedButton.icon(
                    onPressed: () =>
                        _performPowerAction(provider, L18nKeys.sleep),
                    icon: const Icon(Icons.bedtime),
                    label: Text(_tr(L18nKeys.sleep)),
                ...
                Expanded(
                  child: OutlinedButton.icon(
                    onPressed: () =>
                        _performPowerAction(provider, L18nKeys.hibernate),
                    icon: const Icon(Icons.power_settings_new),
                    label: Text(_tr(L18nKeys.hibernate)),
                    style: OutlinedButton.styleFrom(
                      padding: const EdgeInsets.symmetric(vertical: 12),
                    ),
                  ),
  ...
  Future<void> _changePowerMode(
    ...
    final confirm = await showAdvancedConfirmDialog(
      context: context,
      title: l10n.translate(L18nKeys.changePowerMode),
      content:
          '${l10n.translate(L18nKeys.switchToPowerMode).replaceAll('{mode}', l10n.translate(mode.nameKey))}\n\n${l10n.translate(mode.descriptionKey)}',
    ...
    if (confirm == true) {
      ...
      if (success && mounted) {
        _showSuccess(
          _tr(
            L18nKeys.powerModeChanged,
          ).replaceAll('{mode}', _tr(mode.nameKey)),
        );
      }
    }
  }
  ...
  Future<void> _changePowerPlan(
    ...
    if (success && mounted) {
      _showSuccess(
        _tr(L18nKeys.powerPlanChanged).replaceAll('{plan}', plan.name),
      );
    }
  }
  ...
  Future<void> _performPowerAction(
    ...
    final l10n = AppLocalization.of(context);
    final action = l10n.translate(actionKey);
    final contentKey = actionKey == L18nKeys.sleep
    ...
    final confirm = await showAdvancedConfirmDialog(
      context: context,
      title: action,
      content: l10n.translate(contentKey),
      icon: actionKey == L18nKeys.sleep
          ? Icons.bedtime
    ...
    if (confirm == true) {
      bool success = false;
      if (actionKey == L18nKeys.sleep) {
        success = await provider.sleep();
      } else if (actionKey == L18nKeys.hibernate) {
      ...
      if (!success && mounted) {
        _showError(
          l10n
              .translate(L18nKeys.failedToAction)
